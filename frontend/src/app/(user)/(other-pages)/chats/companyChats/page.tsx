...
export default function ChatsPage() {
  ...
  useEffect(() => {
    userSocket.on("call-failed", ({ reason }) => {
      if (reason == "USER_OFFLINE") {
        enqueueSnackbar("User is offline, please try again later", {
          variant: "error",
        });
      }
      ...
      if (reason == "USER_BUSY") {
        enqueueSnackbar(
          "User is on another call right now, please try after some time",
          { variant: "error" }
        );
      }
      ...
      if (reason == "IN_ANOTHER_CALL") {
        enqueueSnackbar(
          "You are already in another call, please try after cancelling the current call",
          { variant: "error" }
        );
      }
    });
  ...
  const selectedUser = conversations?.find((c) => c.users === selectedUserId);
    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      {callOptionsModal && (
        <CallOptionsPopup
          ...
          onCallNow={videoCallUser}
          onScheduleLater={openScheduleModal}
          onClose={() => setCallOptionsModal(false)}
        />
      )}
      ...
      {callScheduleModal && (
        <ScheduleCallModal
          ...
          onClose={() => setCallScheduleModal(false)}
        />
      )}
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        <div className="flex-1 flex overflow-hidden gap-4 p-4 max-w-7xl mx-auto w-full min-h-0">
          <div
            ...
            } md:flex w-full md:w-80 bg-white rounded-lg shadow-sm border border-gray-200 flex-col overflow-hidden min-h-0`}
          >
            ...
            {companyExists ? (
                ...
                <div className="hidden md:flex flex-1 bg-white rounded-lg shadow-sm border border-gray-200 flex-col items-center justify-center min-h-0">
                  <div className="text-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">
                      You cannot access your company right now
                    </h3>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p
                            className={`text-sm ${
                              user?.lastMessage?.content?.isRead
                                ? "text-gray-500"
                                : "text-black"
                            }  truncate`}
                          >
                            {user?.lastMessage?.content?.message}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="hidden md:flex flex-1 bg-white rounded-lg shadow-sm border border-gray-200 flex-col items-center justify-center min-h-0">
                  <div className="text-center">
                    ...
                    <p className="text-gray-500">
                      Start a conversation by messaging one of your connections
                    </p>
                  </div>
                </div>
              ))
              ...
              <div className="hidden md:flex flex-1 bg-white rounded-lg shadow-sm border border-gray-200 flex-col items-center justify-center min-h-0">
                <div className="text-center">
                  <div className="text-6xl mb-4">💬</div>
                  <h3 className="text-xl font-semibold text-gray-900 mb-2">
                    You don&apos;t have a active company
                  </h3>
