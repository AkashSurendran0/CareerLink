...
export default function CreateJobPage() {
  ...
  const handleArrayChange = (index: number, value: string, setter: any) => {
    const newArray = [...setter]
  ...
  const removeField = (index: number, setter: any) => {
    setter((prev) => prev.filter((_, i) => i !== index))
  }

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    console.log("Job posting submitted:", { formData, skills, responsibilities, benefits })
  }
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600 text-sm">
                      Specify the skills, experience, and education required for the job.
                    </p>
                  </div>
                </div>
                    ...
                    {skills.map((skill, index) => (
                      <div key={index} className="flex gap-2 mb-2">
                        <input
                          type="text"
                          value={skill}
                          onChange={(e) => handleArrayChange(index, e.target.value, setSkills)}
                    ...
                    {responsibilities.map((resp, index) => (
                      <div key={index} className="flex gap-2 mb-2">
                        <input
                          type="text"
                          value={resp}
                          onChange={(e) => handleArrayChange(index, e.target.value, setResponsibilities)}
                          placeholder="e.g., Java, Python, React"
                          className="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        />
                    ...
                    {benefits.map((benefit, index) => (
                      <div key={index} className="flex gap-2 mb-2">
                        <input
                          type="text"
                          value={benefit}
                          onChange={(e) => handleArrayChange(index, e.target.value, setBenefits)}
