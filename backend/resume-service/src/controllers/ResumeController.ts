...
export class ResumeController {
    ...
    createResume = async (req: Request, res: Response) => {
        try {
            ...
            let result = await this._checkNormalVersion.checkResume((subscription?.data as any)?.result, id);
            if (!result.success) {
            ...
            const base64Pdf = (createResult as any).pdf.toString("base64");
            const finalResult = { success: createResult.success, html: (createResult as any).html, base64Pdf };
    ...
    getTailoredResume = async (req: Request, res: Response) => {
        try {
            ...
            const subscription = await axios.get(`${process.env.API_GATEWAY_ROUTE}/subscription/v1/getUserPlan?user=${id}`);
            let result = await this._checkTailoredVersion.checkTailoredResume((subscription.data as any).result, id);
            if (!result.success) {
                return res.json({ result });
            }
    ...
    getTailoredCoverLetter = async (req: Request, res: Response) => {
        try {
            ...
            const token = req.cookies?.token;
            const subscription = await axios.get(`${process.env.API_GATEWAY_ROUTE}/subscription/v1/getUserPlan?user=${id}`);
            let result = await this._checkTailoredVersion.checkTailoredCoverLetter((subscription.data as any).result, id);
            if (!result.success) {
                return res.json({ result });
            }
            const jobDetails = await axios.get(`${process.env.API_GATEWAY_ROUTE}/job/v1/getJobDetails?id=${job}`);
