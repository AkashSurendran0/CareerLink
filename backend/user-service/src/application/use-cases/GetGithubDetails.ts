...
export class GetGithubDetails implements IGetGithubDetails {
    ...
    async getGithubDetails(user: string): Promise<any> {
        try {
            ...
            if (cachedDetails) details = JSON.parse(cachedDetails);
            ...
            if (!details) {
                ...
                // @ts-ignore
                const totalStars = repoInfo.data.reduce((acc: number, repo: any) => acc + repo.stargazers_count, 0);
    ...
    async getGithubHeatmap(user: string): Promise<any> {
        try {
            ...
            if (cachedHeatmap) heatmap = JSON.parse(cachedHeatmap);
